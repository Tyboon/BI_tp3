 ...
 * Les données sont des n-uplets de nombres réels (double), n est le nombre de dimensions de la donnée
 *
...
public class Cluster implements java.lang.Iterable<Donnee>{
    ...
    public void add(Donnee d) throws ClusterException{
        if (this.dimDonnee != d.nbDimensions()) 
        	throw new ClusterException("les donnees doivent avoir toutes la même dimension");
        this.data.add(d) ;
        this.nb++ ;
    ...
    public double wc() throws ClusterException{ //TODO done
        double som = 0.0 ;
        Distance euclid = new DistanceEuclidienne();
        Donnee barycentre = this.moyenne();
        for (Donnee d : this.data){
        	som += euclid.valeur(d, barycentre);
        }
        return som ;
    }
